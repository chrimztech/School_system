...
import { Plus, Pencil, Trash2, Loader2, Building2, ChevronDown, ChevronRight, BookMarked, ArrowRightLeft, UserCog } from "lucide-react";
...
function DepartmentsPage() {
  ...
  const DeptDialog = ({ open, onClose, title }: { open: boolean; onClose: () => void; title: string }) => (
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid gap-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Name *</Label>
              <Input className="mt-1" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Sciences" maxLength={60} />
            </div>
            <div>
              <Label>Code</Label>
              <Input className="mt-1" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="SCI" maxLength={10} />
            </div>
          </div>
          <div>
            <Label>Head of Department</Label>
            <Input className="mt-1" value={form.head} onChange={(e) => setForm({ ...form, head: e.target.value })} placeholder="Mr. Banda" maxLength={80} />
          </div>
          <div>
            <Label>Description</Label>
            <Textarea className="mt-1 min-h-16 resize-none" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Brief description…" maxLength={200} />
          </div>
        </div>
  ...
  const SubjectRow = ({ s, currentDept }: { s: any; currentDept: string }) => (
    <div className="flex items-center justify-between px-4 py-2 border-t border-border/50 bg-muted/20 hover:bg-muted/40 transition-colors">
      <div className="flex items-center gap-3">
        ...
        {s.phase && (
          <Badge variant="outline" className="text-xs">{PHASE_LABEL[s.phase] ?? s.phase}</Badge>
        )}
        {(s.compulsory ?? s.isCore) && (
      ...
      <Button
        ...
        onClick={() => setReassigning({ subject: s, targetDept: currentDept })}
      >
  ...
  return (
    <div className="space-y-6">
      ...
      <DeptDialog open={addOpen} onClose={() => setAddOpen(false)} title="Add department" />
      <DeptDialog open={!!editTarget} onClose={() => { setEditTarget(null); setForm(emptyForm()); }} title={`Edit — ${editTarget?.name ?? ""}`} />
                  ...
                  <div>
                    ...
                    {deptSubjects.length === 0 && deptTeachers.length === 0 ? (
                      ...
                      deptSubjects.map((s: any) => (
                        <SubjectRow key={s.id} s={s} currentDept={d.name} />
                      ))
                    )}
                  </div>
                )}
                  ...
                  {unassigned.length > 0 && (
                    <>
                      ...
                      {unassigned.map((s: any) => (
                        <SubjectRow key={s.id} s={s} currentDept="" />
                      ))}
                    </>
                  )}
                </div>
