...
// configured via environment variables in the live deployment; every other provider lists here
// as a platform-wide fallback/default, used when a school hasn't connected its own account (see
// each school's own Integrations page for the per-school path, which every provider also has).
function PlatformIntegrationCard({ schema }: { schema: (typeof PROVIDER_SCHEMAS)[number] }) {
  const qc = useQueryClient();
  const { data: configs = [] } = useQuery({
    queryKey: ["platform-integration-configs"],
    queryFn: () => api.integrationConfigs.listPlatform(),
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`${schema.name} settings saved`);
      setOpen(false);
    },
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Platform's own account — the fallback for a school that hasn't connected its own (see that school's Integrations page).
            </p>
          </div>
        </div>
        <Chip size="small" label={config?.enabled ? "Enabled" : "Disabled"} sx={badgeSx(config?.enabled ? "success" : "secondary")} />
      </div>
      <div className="mt-4">
        <Button size="small" variant="outlined" onClick={() => setOpen(true)}>Configure</Button>
      </div>
      {open && (
      ...
      {tab === "gateways" && (
        <Box className="grid gap-4 lg:grid-cols-2">
          <p className="col-span-full text-sm text-muted-foreground">
            Configure the platform's own account for each provider here instead of redeploying with
